...
public class PlayerController : Player, IPunObservable
{
        ...
        if (canControl) {
            ...
            if (!isSeeker) {
                ...
                if (Input.GetKeyDown(KeyCode.R))
                    photonView.RPC("TransformToPlayer", RpcTarget.AllBuffered);
            }

            //Open/Close door
            if (_input.use && doorDetected)
                doorPV.RPC("DoorState", RpcTarget.AllViaServer);
    ...
    public void PlayFootStep()
    {
        if (photonView.IsMine)
        {
            ...
            {
                photonView.RPC("ChangeFootstepsVolume", RpcTarget.All, 0.5f, 10);
            }
            else if (_input.crouch && audioSource.volume != 0.2f)
            {
                photonView.RPC("ChangeFootstepsVolume", RpcTarget.All, 0.2f, 4);
            }

            if (audioSource && grounded)
            {
                audioSource.PlayOneShot(gs.concreteFootsteps[Random.Range(0, gs.concreteFootsteps.Length)]);
                photonView.RPC("RPC_PlayFootstep", RpcTarget.Others);
            }
        }
    }
    ...
    public void TakeDamage(int dmg, string killerName)
    {
        ...
        if (health <= 0 && !isDead)
            Die(killerName);
            //photonView.RPC("Die", RpcTarget.AllBuffered, killerName);
    }
    ...
    void Die(string killerName)
    {
        ...
        isDead = true;
        canControl = false;
        //gm.allPlayers.Remove(PhotonNetwork.GetPhotonView(photonView.ViewID).GetComponent<PlayerController>());
        if (photonView.IsMine)
        {
            photonView.RPC("TransformToPlayer", RpcTarget.AllBuffered);
    ...
    void TransformToObject(int targetID)
    {
        ...
        if (targetObj != null)
        {
            photonView.RPC("DisableColliders", RpcTarget.AllBuffered);
            ...
            isTransformed = true;
            photonView.RPC("SetupObject", RpcTarget.AllBuffered, transformID, photonView.ViewID, targetID, yOffset);
        }
    }
